...
#     x_train[index] = scaler.fit_transform(np.array(x_train[index])[0].reshape(-1, 1))
# for index in range(n):
#     scaler = StandardScaler()
#     x_test[index] = scaler.fit_transform(np.array(x_test[index])[0].reshape(-1, 1))

print('训练数据总量', m)
init_l = 60
_, x_train_l, _, y_train_l = train_test_split(x_train, y_train, test_size=init_l/m)
...
for it in list(range(left, right, dist)):
    ...
    for i in range(10):
        ...
        # 对比试验 高斯模型
        x_train_p, y_train_p = a_gpr.sample_point(x_train, y_train, iter=it, is_init=True)
        model_gpm = a_gpr.creat_gpr_model(x_train_p, np.array(y_train_p).reshape(-1, 1))
        y_pre_gpm = [a_gpr.predict_mu_var(np.reshape(x_test[i], (1, -1)), model_gpm, re_var=False) for i in range(np.shape(x_test)[0])]


        list_mse_gpr.append(mean_squared_error(y_test, y_pre_gpr))
        list_mse_gpm.append(mean_squared_error(y_test, y_pre_gpm))
    ...
    list_mean_mse_gpm.append(np.mean(list_mse_gpm))
...
plt.ylabel('w_hf')
plt.xlabel('iter')
plt.title('boston_price%s'%init_l)
...
plt.plot(list(range(left, right, dist)), list_mean_mse_gpm, lw=1.5, label='gpm_m')
...
gp_model = GPRegression(x_train, np.reshape(y_train, (-1, 1)), kernel=k_rbf)
