...
df_1 =pd.DataFrame(data=x_data, columns=None)
...
# _, x_tran_l, _, y_train_l = train_test_split(x_train, y_train, test_size=0.05)

x_train_l, y_train_l = a_gpr.sample_point(x_train, y_train, iter=20)
...
# train_size = list(np.linspace(0.5, 1, 10))
...
for it in list(range(left, right, dist)):
    ...
    for i in range(10):
        ...
        hf_gp, list_w_hf, x_gp, y_gp = a_gpr.creat_gp_model(max_loop=temp_iter, x_init_l=x_train_l, y_init_l=y_train_l, x_init_h=x_init, y_init_h=y_init,
                                                n_start=1, n_single=200, x_conda=np.array(x_train, ndmin=2), y_conda=np.array(y_train).reshape(-1, 1)
                                                            )
